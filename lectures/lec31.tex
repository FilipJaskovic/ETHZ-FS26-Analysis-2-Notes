...
\begin{remark}[Expression of the induced $n-1$ form with basis forms]\label{rem:n1-forms-vecf}
	...
	\begin{align*}
		\alpha_{F} = \sum_{i=1}^{n} (-1)^{i+1} F_{i}(x) \, d x_{1} \wedge  \dots \wedge \widehat{dx_{i}} \wedge \dots \wedge dx_{n},
	\end{align*}
...
\begin{proof}
	...
	Now we move into a local area on the manifold and prove the theorem there, then it extends trivially to the whole thing. Take a parametrisation $\phi: V \to \partial U$ that by the proof of \ref{prop:norm-orient} satisfies
	\begin{align*}
...
\begin{definition}[Parametrised submanifold with boundary]\label{def:param-subm-boundary}
	...
	A word on orientations (ask the professor):
	The orientation of $M$ is inherited from $\phi(V)$. $\partial M$ has its orientation induced by $\partial U$. That is, at some point $p = \phi(y) \in \partial M$, the unit normal is the normalised vector $D\phi(y) \nu(y)$, where $\nu(y)$ is the normal to $\partial U$ at $y \in \partial U$.
\end{definition}
...
\begin{proof}
	...
	\end{align*}
	And (I suppose) since $\partial M$ is constructed to be positively oriented via $\phi$, nothing stops us from applying the definition to get
	\begin{align*}
		\int_{\partial U} \phi^*\alpha = \int_{\partial M} \alpha.
	\end{align*}
