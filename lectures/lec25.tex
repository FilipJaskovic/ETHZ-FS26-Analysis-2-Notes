...
\chapter{Global integration theorems}
...
\begin{definition}[Bounded $C^{k}$ domain]\label{def:bdd-ck-dom}
	...
	Furthermore, we demand that $\partial \Omega$ is graphical. So for every point $p_{0} \in \partial \Omega$, there exists an isometry $F: \mathbb{R}^{n} \to \mathbb{R}^{n}$ and a ball $B_{r}(p_{0})$ such that
	$$
		F(\partial \Omega) \cap B_r(p_{0})=\left\{\left(x^{\prime}, x_n\right) \in V \times \mathbb{R} \mid x_n=g\left(x^{\prime}\right)\right\} \cap B_r(p_{0}) ;
	$$
	and
	$$
		F(\Omega) \cap B_r(p_{0})=F\left(\Omega \cap B_r(p_{0})\right)=\left\{\left(x^{\prime}, x_n\right) \in V \times \mathbb{R} \mid x_n<g\left(x^{\prime}\right)\right\} \cap B_r(p_{0}).
	$$
	Unless specified otherwise, we assume that $F = \text{id}$ for simplicity. The most general case will only come into play once we introduce graphical covers of submanifolds.
	% todo: picture of a graphical domain like this
\end{definition}
...
\begin{lemma}[Exterior unit normal]\label{lem:ext-unit-norm}
	Let $\Omega$ be a bounded $C^{1}$ domain, $p_{0} \in \partial \Omega$ the point we're focusing on and assume $\phi: V \to \mathbb{R}^{n}$ defined by $\phi(y) = (y, g(y))$ where $g: V \to \mathbb{R}$, with $V \subset \mathbb{R}^{n-1}$ open, is a graphical $C^{1}$ parametrisation of $M := \partial \Omega \cap B_{r}(p_{0})$.

	Define for $p = (y,g(y))$ the vector
	\begin{align*}
...
\begin{proof}
	...
	To prove it is exterior, consider that if we need to show $(y', y_{n}) = (y,g(y)) + h (-\nabla g(p), 1) \not \in \overline{ \Omega } $. For that, we would have to show that $y_{n} > g(y')$ (precisely because $\Omega$ is a $C^{1}$ domain that is \textit{not} rotated by $F$). But we can do a Taylor expansion (check details)
	\begin{align*}
...
\begin{definition}[Exterior unit map]\label{def:ext-unit-map}
	If $\Omega \subset \mathbb{R}^n$ is a bounded $C^1$ domain, the continuous map $\nu: \partial \Omega \rightarrow \mathbb{S}^{n-1}$ assigning to each point $p \in \partial \Omega$ the exterior normal vector at $p$ is called exterior unit normal map (if it exists, we will repeatedly use \ref{lem:ext-unit-norm} for the simplified domain). Notice that since the tangent space at a point of a manifold is defined uniquely, the map is unique to each manifold.
\end{definition}
...
This will be the setup for every Proposition in the following section. Let $\Omega$ be a bounded $C^{1}$ domain, $p_{0} \in \partial \Omega$. Consider the locally graphically parametrised manifold $M := \partial \Omega \cap B_{r}(p_{0})$
and let $\nu$ denote its unit exterior normal map (exists by \ref{lem:ext-unit-norm}). From the definition of $C^{1}$ domain, we also have
\begin{align*}
