...
\begin{definition}[$k$-form in $\mathbb{R}^{n}$]\label{def:k-form}
    ...
    In fact, this is so important and common that we define the "simple" $k$-forms $\omega_{I}$ which assign each point $p \in U$ the map $\omega_{I}: T \mapsto \det(e_{I}^\top T)$ for each $I \in \mathcal{I}_{k}$ and express any other $k$-form as a linear combination of these.
\end{definition}
...
\begin{definition}[Conventions about $k$-forms]\label{def:conv-forms}
    ...
    \end{align*}
    and we demand that each $c_{I}(x)$ is a $C^{\infty}$ function in $x$.
...
\begin{definition}[Differential]\label{def:differential-forms-version}
    ...
    \begin{align*}
        (d\alpha )_{x} := \sum_{I \in \mathcal{I}_{k}} dc_{I}(x) \wedge \omega_{I},
    \end{align*}
    where we remind ourselves that $dc_{I}(x): v \mapsto (Dc_{I})_{x} (v)$ and it is a $1$-form, so each $dc_{I}(x) \wedge \omega_{I}$ is a $k+1$-form.
\end{definition}
...
\begin{remark}[Relation to old notation]\label{rem:k-forms}
    ...
    \begin{align*}
        \alpha = \sum_{I \in \mathcal{I}_{k}} c_{I} \, dx_{I}
    \end{align*}
...
\end{lemma}
\begin{proof}
    It suffices to check this at the basis vectors, so compute $d d(f(x) \omega_{I})$ for $f(x) = x_{i}$. 
    \begin{align*}
        d f \omega_{I} = \sum_{i=1}^{n} \partial_{i}f \, dx_{i} \wedge \omega_{I} \implies d(df) = \sum_{i=1}^{n}  \sum_{j=1}^{n} \partial_{j} \partial_{i} f \, dx_{j} \wedge dx_{i} \wedge \omega_{I}.
    \end{align*}
