...
\begin{proof}
	...
	\end{align*}
	where we used the fact that the determinant is alternating (verify as an exercise). So we commute at the cost of the sign and get the statement.
\end{proof}
...
\begin{lemma}[Pullback of basis form]\label{lem:pullback-basis}
	Let $U \subset \mathbb{R}^{n}$ open. Assume $\alpha = e_{I}^* \in \Omega^{k}(U)$ for $I = (i_{1}, \dots, i_{k}) \in \mathcal{I}_{k}$ is a $k$-form and $F(y) = (F_{1}(y), \dots, F_{n}(y))$ such that $F \in C^{\infty}(U)$. Then
	\begin{align*}
		F^*\alpha = F^*e_{I}^* = dF_{i_{1}} \wedge \cdots \wedge dF_{i_{k}}.
	\end{align*}
	Sometimes we write $F^*dx_{I} = dF_{i_{1}} \wedge \cdots \wedge dF_{i_{k}} =: dy_{i_{1}} \wedge \cdots \wedge dy_{i_{k}}$.
\end{lemma}
\begin{proof}
	We have by definitions, noticing that $dF_{i}(y) = DF_{i}(y) = (\partial_{1} F_{i}(y), \dots, \partial_{n} F_{i}(y))$ and $e_{I}^*(DF) = DF_{i}$,
	\begin{align*}
...
\begin{proof}
	...
	\begin{align*}
		F^*(d\alpha ) = F^*(df) \wedge F^*(dx_{i_{1}}) \wedge \cdots \wedge F^*(dx_{i_{k}}) = F^*(df) \wedge dF_{i_{1}} \wedge \cdots \wedge dF_{i_{k}}.
	\end{align*}
	By the chain rule and since $F^* f = f \circ F$,
	\begin{align*}
		F^*(df)_{y}(T) = df_{F(y)}(DF(y) \cdot T) = Df_{F(y)} DF(x) \cdot T = D(f \circ F)_{y}T = d(F^* f)_{y} (T).
	\end{align*}
...
\begin{definition}[Orientable submanifold]\label{def:orientable-subm}
	A $k$-submanifold $M \subset \mathbb{R}^{n}$ is called orientable if it is covered by local parametrisations with transition maps that have positive Jacobi determinants. That is, for any two local parametrisations $\phi_{1}, \phi_{2}$ that cover different parts of the manifold $M \cap U_{1}$ and $M \cap U_{2}$ respectively, then the reparametrisation $\psi = \phi_{2}^{-1}  \circ \phi_{1}$ for $M \cap U_{1} \cap U_{2}$ has positive Jacobi determinant.
\end{definition}

\begin{remark}[Criterion in $3$D]
	In $\mathbb{R}^{3}$, a $2$-submanifold $M \subset \mathbb{R}^{3}$ is orientable if and only if there exists a normal vector map $\exists \nu: M \to \mathbb{S}^{2} \subset \mathbb{R}^{3}$, $\nu(p) \perp T_{p}M$ and $\nu$ is continuous. See exercise sheet.
\end{remark}

\begin{definition}[Integral over parametrised submanifolds using $k$-forms]\label{def:int-param-subm-k-form}
	Let $V \subset \mathbb{R}^{k}, U \subset \mathbb{R}^{n}$ open and $\phi: V \to U$ be a parametrised submanifold $M = \phi(V)$ that is orientable. Let $\alpha \in \Omega^{k}(U)$. We define
	\begin{align*}
		\int_{M} \alpha := \int_{V} \alpha (D \phi(x)) \, dx.
	\end{align*}
\end{definition}
